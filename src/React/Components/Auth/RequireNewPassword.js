...
const RequireNewPassword = ({
  ...
  const [usernameValid, setUsernameValid] = useState(!!initialUsernameValue);
  ...
  useEffect(() => {
    setIsValid(
      passwordValid &&
      usernameValid &&
      Object.entries(customsValid).every(([key, value]) => !!value || !order.includes(key))
    );
  }, [usernameValid, passwordValid, JSON.stringify(customsValid), order.length]);
